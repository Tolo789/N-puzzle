...
static std::vector<Node *>::iterator getBestNodePosition(std::vector<Node *> &myVector, Node &node) {
	if (myVector.size() > 0 && **(myVector.end() - 1) < node)
		return myVector.end();
	std::vector<Node *>::iterator it = myVector.begin();

	while (it != myVector.end() && **it < node) {
		it++;
	}
		...
		while (i < 4) {
			if ((i == 0 && tmpNode->points[0].y_current != 0) ||						// up
				...
				if (it == closedList.end()) {
					...
					if (it == openList.end()) {
						// it = getBestNodePosition(openList, *newNode);
